...
def plot_f_error(y_off, ax):
    ...
    ax.set_xlabel(r'$\varepsilon_{f}$', fontsize=12)
...
def plot_amplitude_error_dist(sign_v, a_error_dist, ex, ex_i0, ex_i1, ax):
    ...
    ax[-1].set_ylabel(r'$\varepsilon_{S}$', fontsize=12)
    ax[-1].set_xlabel(r'field difference ($\Delta S$)', fontsize=12)
...
def main():
    ...
    gs = gridspec.GridSpec(1, 2, left=0.15, bottom=0.65, right=0.9, top=0.95, width_ratios=[2.5, 1], wspace=0.2)
    ax0 = []
    ax0.append(fig.add_subplot(gs[0, 0]))
    ax0.append(fig.add_subplot(gs[0, 1]))


    gs = gridspec.GridSpec(3, 4, left=0.15, bottom=0.1, right=0.9, top=0.5, hspace=0.4, wspace=0.4, height_ratios=[2, 2, 1.5])
    ax = []
    ...
    fig.tag(axes=[ax0[0], ax[0], ax[-1]], labels=['A', 'C', 'D'], fontsize=15, yoffs=2, xoffs=-8)
    fig.tag(axes=[ax0[1]], labels=['B'], fontsize=15, yoffs=2, xoffs=-4)


    plt.savefig('tracking_features_new.pdf')
    plt.show()
