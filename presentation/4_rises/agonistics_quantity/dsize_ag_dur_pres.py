...
def plot_dsize_chase(mm_lose_rc, ff_lose_rc, mf_lose_rc, fm_lose_rc,
                  mm_win_dsize, ff_win_dsize, mf_win_dsize, fm_win_dsize, mix_win_dsize, ax, select_i = None, markersize=8, alpha=1):
...
def main():
    ...
    quit()
    ###########################
    fig = plt.figure(figsize=(12 / 2.54, 12 * (12 / 20) / 2.54))
    gs = gridspec.GridSpec(1, 1, left=0.16, bottom=0.25, top=0.95, right=0.95)
    ax = fig.add_subplot(gs[0, 0])

    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax, select_i=[1], markersize=7)
    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 2, 3], markersize=7, alpha=0.2)
    ...
    regression_line(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 2], color=male_color, alpha=0.2)
    ...
    ax.set_ylabel('chase duration [s]', fontsize=fs+2)
    ax.tick_params(labelsize=fs)

    plt.savefig('pres_chase_dsize2.png', dpi=300)
    plt.show()
    ...
    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax, markersize=7)
    # regression_line(*lose_rc, *dsize_win, ax, select_i=[0, 2], color=male_color)
    # regression_line(*lose_rc, *dsize_win, ax, select_i=[1, 3], color=female_color)
    regression_line(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 2], color=male_color)
    ax.plot([0, 0], [-50, 500], 'grey', linestyle='dashed', lw=1.5)
    ...
    plt.savefig('poster_chase_dsize.pdf')
    plt.show()
    ##############################
    ...
    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax)
    # regression_line(*lose_rc, *dsize_win, ax, select_i=[0, 2], color=male_color)
    # regression_line(*lose_rc, *dsize_win, ax, select_i=[1, 3], color=female_color)
    regression_line(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 1, 2])

    ax.set_xlabel(r'$\Delta$size [cm]', fontsize=fs+2)
    ax.set_ylabel('chase duration [s]', fontsize=fs+2)
    ax.tick_params(labelsize=fs)

    plt.savefig('dsize_chase_all.jpg', dpi=300)
    ##############################
    ...
    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 1, 2])
    regression_line(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 1, 2])
    ...
    plt.savefig('dsize_chase_fm_excl.jpg', dpi=300)
    ##############################
    ...
    plot_dsize_chase(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 2])
    regression_line(*med_agonstic_dur, *dsize_win, ax, select_i=[0, 2])

    ax.set_xlabel(r'$\Delta$size [cm]', fontsize=fs+2)
    ax.set_ylabel('chase duration [s]', fontsize=fs+2)
    ax.tick_params(labelsize=fs)

    plt.savefig('dsize_chase_male_win.jpg', dpi=300)
