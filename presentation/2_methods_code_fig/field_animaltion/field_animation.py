...
def load():
    # folder = "../data/2016-04-10-11_12/"\
    folder = "/home/raab/writing/thesis/chapter_2_methods/data/2016-04-10-11_12"
...
def main():
    ...
    #####################
    ioi = np.arange(len(times))[times >= 50][0]
    fig = plt.figure(figsize=(15/2.54, 5/2.54))
    gs = gridspec.GridSpec(1, 3, left=0.005, bottom = 0.005, right=0.995, top=0.975, hspace=0.3)
    ax = []
    ...
    for enu, s, f, in zip(np.arange(3), sign_oi, fund_oi):
        ax[enu].imshow(s[ioi].reshape(8, 8)[::-1], cmap='jet', interpolation='gaussian')

        for x, y in zip(X, Y):
            ax[enu].plot(x, y, '.', color='k', markersize=4)

    for a in ax:
        a.set_xticks([])
        a.set_yticks([])
    plt.savefig('field_properties_ex.jpg', dpi=300)
    plt.show()
    quit()
    ############################
    ...
    gs = gridspec.GridSpec(2, 6, left=0.005, bottom = 0.005, right=0.995, top=0.975, hspace=0.3)
    ax = []
    ...
    ax_trace.set_ylabel('frequency [Hz]', fontsize=fs+2)
    ax_trace.set_xlabel('time [s]', fontsize=fs+2)
    ax_trace.tick_params(labelsize=fs)
